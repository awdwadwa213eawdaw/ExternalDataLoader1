return function(_p)
	...
	return {
		...
		ElekiRocks = function(model)
			if not _p.MasterControl.WalkEnabled then return end
			local chat = _p.NPCChat
			_p.MasterControl.WalkEnabled = false
			_p.MasterControl:Stop()
			spawn(function() _p.MasterControl:LookAt(model.rock.Position) end)
			local headbutter, done, proceed
			Utilities.fastSpawn(function()
				headbutter = _p.Network:get('PDS', 'getHeadbutter')
				proceed =  _p.Network:get('PDS', 'ElekiCheck')
				done = true
			end)
			chat:say('This boulder seems to be loose.')
			while not done do wait() end
			if headbutter and chat:say('[y/n]Would you like ' .. headbutter .. ' to use Headbutt?') then
				chat:say(headbutter .. ' used Headbutt!')
				if proceed then
					Utilities.sound(9118613455, 1.5, nil, 5)
					model.rock.Position = model.rock.Position + Vector3.new(2,0,0)
					wait(.5)
					model.rock.Position = model.rock.Position - Vector3.new(2,0,0)
					chat:say('The boulder just barely moved, but something seems to be angered by your action.')
					--//Cutscene
					local Cloud = script.StormCloud:Clone()
					Cloud.Parent = workspace
					Cloud.CFrame =  _p.player.Character.HumanoidRootPart.CFrame * CFrame.new(0,90,0)
					Utilities.sound(133426162, 0.5, nil, 10)
					Utilities.exclaim(_p.player.Character.Head)
					local CCamera = workspace.CurrentCamera
					local LegacyCF = CCamera.CFrame
					wait(1)
					CCamera.CameraType = Enum.CameraType.Scriptable
					game:GetService("TweenService"):Create(CCamera, TweenInfo.new(0.8, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {CFrame = CFrame.new(LegacyCF.Position, Cloud.Position)}):Play()
					wait(1.5)
					chat:say("A lightning cloud appeared above you!")
					Utilities.sound(1117093820, 2, nil, 10)
					game:GetService("TweenService"):Create(CCamera, TweenInfo.new(.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {CFrame = LegacyCF}):Play()
					local Thunder = script.Lightning:Clone()
					Thunder.Parent = workspace
					Thunder.CFrame =  _p.player.Character.HumanoidRootPart.CFrame * CFrame.new(-6,35,0)
					game.Debris:AddItem(Thunder, 0.1)
					task.spawn(function()
						model.Parent.ElekiBlock:Destroy()
						for i, v in pairs(model.Parent.DestroyedElekiRocks:GetChildren()) do
							v.Anchored = false
							v.CanCollide = false
							wait()
						end
					...
					wait(1)
					chat:say(proceed.."\'s Lightning Rod ability saved you from the attack!",
						"The rocks infront of you collapsed to the lightning!"
					)
					_p.PlayerData:completeEvent('RegiElekiRocksDown')
					
				else
					chat:say(headbutter..' couldn\'t get the boulder to budge.')
				end
...
--[[	SantaClaus = function(model) -- to make it easy to unify the 3 santas in different chunks
		...
		mashableMineral = function(model)
			...
			local isKey = model.Name == 'KeyRockSmash'
			chat:say(smasher .. ' used Rock Smash!')
			Utilities.sound(9125869521, 1, nil, 5)
			--		pcall(function() model['#InanimateInteract']:Destroy() end)
		...
		Regidrago = function(model)
			if not _p.PlayerData.completedEvents.CompletedCatacombs or not _p.PlayerData.completedEvents.ResAndZek then return end
			if not _p.MasterControl.WalkEnabled then return end
			local chunk = _p.DataManager.currentChunk
			if not chunk or chunk.id ~= 'chunk204' then return end
			local enc
			pcall(function() enc = chunk.regionData.Regidrago end)
			if not enc then return end
			...
			delay(3, function() model:Destroy() end)
			_p.Battle:doWildBattle(enc, {musicId = 10840573719})
			pcall(function() _p.PlayerData:completeEvent('Regidrago') end)
			_p.MasterControl.WalkEnabled = true
			_p.MasterControl:Stop()
			_p.NPCChat:say('Regidrago can now be found roaming in the wild.')
		end,
