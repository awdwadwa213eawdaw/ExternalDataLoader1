...
return function(_p)--local _p = require(script.Parent)
	...
	return {
			...
			interact[moveTutor.model] = function() 
				...
				local moveTutor = function()
					...
					elseif r == 'eg' then
						...
						return
					elseif r == 'nm' then
						moveTutor:Say("Hmmmm, it seems your pok[e\']mon can't learn any moves at all...", "Check back later with a different pok[e\']mon.")
						return
					end
		...
		onBeforeEnter_House6 = function(room)
			local inGroup = _p.player:IsInGroup(32480262)
		...
		onLoad_chunk3 = function(chunk)
			if _p.PlayerData.badges[7] then
				chunk.map.LooseParts.LabDoor.Position = Vector3.new(-33.105, 130.68, -12.11)
				chunk.map.LooseParts.LabDoor.Orientation = Vector3.new(0,45, 0)
			else
				chunk.map.LooseParts.LabDoor.Position = Vector3.new(-34.105, 130.68, -14.11)
				chunk.map.LooseParts.LabDoor.Orientation = Vector3.new(0,0, 0)
			end
			
			if completedEvents.RegiElekiRocksDown then
				chunk.map.ElekiRock1:Destroy()
				chunk.map.DestroyedElekiRocks:Destroy()
				chunk.map.ElekiBlock:Destroy()
			end
		local Scientist = chunk.npcs.EnterScientist
			interact[Scientist.model] = function()
				if completedEvents.CaptureMewtwo then
					Scientist:Say("Sorry, the lab is closed now.")
					return
				end
				if completedEvents.HasMew then
					if Scientist:Say("[y/n]Do you want to enter the lab") then
						Utilities.FadeOut(.5)
						--                                flying = false
						--                                pWeld:Destroy()
						...
						chunk:destroy()
						-- change chunks
						_p.DataManager:loadChunk('Chunk101')

						--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
						Utilities.Teleport(CFrame.new(-78.5, 51.5, -63.5))
						task.wait(1)
						Utilities.FadeIn(.5)
					end
				end
				if not completedEvents.MewTwoItem2 then
					_p.PlayerData:completeEvent('GetCut')
					_p.PlayerData:completeEvent('BeatTeamRocket')
					print(completedEvents)
					Scientist:Say('Hey kid',
						'you\'re not mean\'t to be down here...',
						'Well I can not just let you in, I need some items first, for research.',
						'first thing on the list is a Twisted Spoon.')
					if Scientist:Say('[y/n]Do you happen to have a Twisted Spoon that I can use?') then
						local hasitems = _p.Network:get('PDS', 'hasmewtwoitems', 'twistedspoon')
						if hasitems.twistedspoon then
							Scientist:Say('Alright that is one item checked off')
							print(completedEvents)
						else
							Scientist:Say('Oh, you do not have one, bummer.')
						end
					else
						Scientist:Say('Ok, well tell me if you find one.')
					end
					return
				end
				if not completedEvents.MewTwoItem3 then
					Scientist:Say('Alright, next thing I need is a Mind Plate.')

					if Scientist:Say('[y/n]Do you happen to have a Mind Plate that I can have?') then
						local hasitems = _p.Network:get('PDS', 'hasmewtwoitems', 'mindplate')
						print(hasitems)
						if hasitems.mindplate then
							Scientist:Say('Wow, alright nice two items checked off.')
						else
							Scientist:Say('Sorry, looks like you do not have one.')
						end
					else
						Scientist:Say('Well come back when you do.')
					end
					return
				end
				if not completedEvents.HasMew then
					Scientist:Say('Alright, last thing before I can let you in is that everyone must have a mew.','This is because mew will balance the frequency of your other pokemon.','this will keep Mewtwo calmed and contained.')
					if Scientist:Say('[y/n]Do you have a mew in your party?') then
						local HasMew = _p.Network:get("PDS","CheckForMew")
						if HasMew then
							Scientist:Say('Alright, you do in fact have a mew, talk to me again if you want to enter the lab.')
						else
							Scientist:Say("Sorry you dont have a mew.")
						end
					end
				end
			end
		...
		onBeforeEnter_ZygardeLab = function(room)
			local checker = room.npcs.ZygardeScientist
			local myName = _p.PlayerData.trainerName
			interact[checker.model] = function()
				if not completedEvents.InitialZygardeDialogue then
					chat:say(checker, 'Hello there.',  
						'You\'re the famous '..myName..".", 
						"I assume you've heard all about legendaries and mythicals roaming around Roria.",
						"Well this Legendary Pok[e\']mon we call Zygarde is rather intriguing.",
						"While this creature is most definetly Legendary, there does not seem to be a single one of them, or lots of this roaming around Roria.."
					)
					checker:LookAt(room.model.ZTube.TestTube.Position)
					chat:say(checker, "Instead, pieces of itself called Zygarde Cells are dispersed around Roria.",
						"These cells can not live on their own, and require the aid of Zygarde Cores to come together and form the Pok[e\']mon Zygarde."
					)
					checker:LookAt(_p.player.Character.HumanoidRootPart.Position)
					chat:say(checker, "Depending on how many cells and cores are combined, there are different formes Zygarde takes based on the progress it has made to full completion.",
						"The 10% forme looks like a pet dog, with an odd hexagonic leash attached to it.",
						"The 50% forme drastically increase in size, taking the shape of a Cobra.",
						"Funnily enough, its natural posture makes it look like a Z from the side!",
						"The 100% forme is a titan, named the protector of the ecosystem.",
						"Offensively, it can pull of feats similar to the 50% forme, but is incredibly bulky, allowing it to withstand anything thrown at it!",
						"Except a bit of ice.....",
						"Anyways, you may have already found some Cells during your journey around Roria.",
						"Maybe some cores, but they are rare and are always moving.",
						"Nonetheless, I want you to have this."
					)
					chat:say("Obtained a Zygarde Cube!.")
					onObtainKeyItemSound()
					chat:say(myName .. " put the Zygarde Cube in the Bag.")
					chat:say(checker, "That Zygarde Cube lets you arrange cells and cores into Zygarde's formes.",
						"If you want to use it, come to me and I'll do it for you.",
						"Its hard to use it on your own, without me and my lab's extraordinary equipment.",
						"Have fun collecting Zygarde Cells!"
					)
					checker:LookAt(room.model.ZTube.TestTube.Position)
					_p.PlayerData:completeEvent('InitialZygardeDialogue')
				else
					chat:say("i forgot to script this xd")
				end
		...
		onBeforeEnter_lighthouse = function(room)
			if not completedEvents.DamBusted then
				...
				if _p.PlayerData.badges[8] then
					if not completedEvents.GetBlueOrb then
		...
		onLoad_chunk54 = function(chunk) 
			local Block = chunk.map.Block
			local noentrywithoutallbadges = chunk.npcs.noentrywithoutallbadges
...
--[[		local lawyer = chunk.npcs.EventLawyer
		...
		onLoad_chunk204 = function(chunk)
			local dracoblock = chunk.map.DracoBlock
			local regidrago = chunk.map.Regidrago
			
			if completedEvents.CompletedCatacombs then
				dracoblock:Destroy()
				--[[if completedEvents.Regidrago then
					regidrago:Destroy()
				end]]
			end
		end,
		...
		onLoad_chunk203 = function(chunk)
			...
			local Bream = chunk.npcs.BreamDev
			...
			TriggerEvent.Touched:connect(function(p)
				...
				TriggerEvent:Destroy()
				spawn(function() _p.Menu:disable() end)
				TweenCamera(game.Workspace.CurrentCamera, 4.2, chunk.map.CamPart.CFrame)
		...
		onLoad_chunk109 = function(chunk)
		...
		onLoad_Chunk101 = function(chunk)
			local function TweenCamera(cam, duration, Cframe)
				local ts = game:GetService("TweenService")
				local info = TweenInfo.new(
					duration,
			...
			-- bmike made this above don't touch you twat
			
			task.wait(1.4)
			print(_p.DataManager.currentChunk.regionData)
			local NpcGio = chunk.npcs.Giovanni
			local NpcGrunt = chunk.npcs.boy1
			local NpcGrunt2 = chunk.npcs.girl1
			local deb = false
			local exitdeb = false
			task.wait(1)
			local exit = chunk.map.Exit



			exit.touched:connect(function(p)
				if not p or not p.Parent or players:GetPlayerFromCharacter(p.Parent) ~= _p.player or not MasterControl.WalkEnabled then return end
				if exitdeb then return end
				exitdeb = true
				Utilities.FadeOut(.5)
				--                                flying = false
				--                                pWeld:Destroy()
				...
				Utilities.Teleport(CFrame.new(-38.18, 112.779, -40.173))
				task.wait(1)
				Utilities.FadeIn(.5)
				exitdeb = false
			end)
			...
			if completedEvents.CaptureMewtwo then
				chunk.maps.Tube.Mewtwo:Destroy()
				NpcGio:Destroy()
				NpcGrunt:Destroy()
				NpcGrunt2:Destroy()
			end    

			local trigger = chunk.map.CutsceneGio
			trigger.touched:connect(function(p)
				if not p or not p.Parent or players:GetPlayerFromCharacter(p.Parent) ~= _p.player or not MasterControl.WalkEnabled then return end
				if deb then return end
				deb = true
				local cam = game.Workspace.CurrentCamera
				MasterControl:WalkTo(Vector3.new(51.85, 36.645, -33.176))
				MasterControl:LookAt(NpcGio.model.HumanoidRootPart.Position)
				MasterControl:Stop()
				MasterControl.WalkEnabled = false
				game.Workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
				TweenCamera(game.Workspace.CurrentCamera, .7, chunk.map.campart.CFrame)
				NpcGio:Say("Mewtwo is looking stable, soon I should be able to use his power")
				spawn(function() Utilities.exclaim(NpcGio.model.Head) end)
				spawn(function() Utilities.exclaim(NpcGrunt2.model.Head) end)
				spawn(function() Utilities.exclaim(NpcGrunt.model.Head) end)
				task.wait(1)
				
				spawn(function() NpcGio:LookAt(_p.player.Character.Head.Position) end )
				spawn(function() NpcGrunt:LookAt(_p.player.Character.Head.Position) end )
				spawn(function() NpcGrunt2:LookAt(_p.player.Character.Head.Position) end )
				NpcGio:Say("Oh great, another vistor I see", "You think you will be able to foil my plans?","Big dreams kid, Go ahead and get out here our team has better stuff to do.","Well alright then, I have no choice but to force you out.","Grunts, get him!")
				NpcGrunt:WalkTo(Vector3.new(54.65, 36.645, -30.237))
				spawn(function() NpcGrunt:LookAt(_p.player.Character.HumanoidRootPart.Position) end)
				MasterControl:LookAt(NpcGrunt.model.HumanoidRootPart.Position)
				NpcGrunt:Say("You will not foil our plans!")
				...
				NpcGrunt:Say("Sorry boss...")
				NpcGrunt:WalkTo(Vector3.new(60.85, 35.145, -25.176))
				NpcGrunt:LookAt(_p.player.Character.HumanoidRootPart.Position)

				NpcGrunt2:WalkTo(Vector3.new(54.65, 36.645, -36.178))
				spawn(function() NpcGrunt:LookAt(_p.player.Character.HumanoidRootPart.Position) end)
				MasterControl:LookAt(NpcGrunt2.model.HumanoidRootPart.Position)

				NpcGrunt2:Say("You might have beaten him, but you will not beat me!")
				...
				NpcGrunt2:Say("Sorry boss...")
				NpcGrunt2:WalkTo(Vector3.new(60.85, 35.145, -39.541))
				NpcGrunt2:LookAt(_p.player.Character.HumanoidRootPart.Position)
				if win then
					NpcGio:Say("Oh well, my worthless grunts failed again.","looks like I have to do the job.")
				end

				NpcGio:WalkTo(Vector3.new(55.65, 36.645, -33.237))
				MasterControl:LookAt(NpcGio.model.HumanoidRootPart.Position)

				NpcGio:Say("I will not be defeated by a kid like you.")
				...
				local win = _p.Battle:doTrainerBattle {
					...
					PreventMoveAfter = true,
					LeaveCameraScriptable = true,
					trainerModel = NpcGio.model,
					num = 219
				}


				NpcGio:Say("I dont even care, this Mewtwo will destroy you anyway.","We are out of here")

				Utilities.FadeOut(.5)
				NpcGio:Destroy()
				NpcGrunt:Destroy()
				NpcGrunt2:Destroy()
				chunk.map.Tube.Mewtwo:Destroy()
				Utilities.FadeIn(.5)
				print(completedEvents)
				_p.PlayerData:completeEvent('CaptureMewtwo')
				chat:say("Mewtwo Attacked!")

				_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Mewtwo)	
			end)
		end,
