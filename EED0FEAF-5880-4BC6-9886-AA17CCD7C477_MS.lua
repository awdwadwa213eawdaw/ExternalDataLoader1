...
function PlayerData:ownsGamePass(passId, mustReturnInstantly)
	...
	local s, r = pcall(function() return self.player:IsInGroup(32480262) end)
	if r then
...
function PlayerData:getzcells()
	local zgarde = _f.Database.ItemById.zygardecell
	local nzgarde = 0
	pcall(function() nzgarde = self:getBagDataByNum(zgarde.num, 1).quantity end)
	return nzgarde
end
...
function PlayerData:spinRouletteForPoke() 
	...
	network:postAll('SystemChat', '🍇', self.player.Name..' just won '..poke.species..'🍇', Color3.fromRGB(165, 38, 207))
	status = false
...
function PlayerData:CheckForMew()
	print(self.party)
	for _, p in self.party do
		if p.num == 151 then self:completeEventServer('HasMew') return true end
	end
	return false
end

function PlayerData:hasmewtwoitems(item)
	if self.completedEvents.CaptureMewtwo then return false end
	...
	if self:getBagDataById("twistedspoon", 1) then items.twistedspoon = true end
	if self:getBagDataById("mindplate", 1) then items.mindplate = true end

	if items[item] then
		if item == 'twistedspoon' then
			if not self.completedEvents.MewTwoItem2 then
				self:completeEventServer('MewTwoItem2')
				self:incrementBagItem(item, -1)
			end
		elseif item == 'mindplate' then
			if not self.completedEvents.MewTwoItem3 then
				if not self.completedEvents.MewTwoItem2 then return false end
				self:completeEventServer('MewTwoItem3')
				self:incrementBagItem(item, -1)
			end
		end
	end
...
function PlayerData:ElekiCheck()
	
	local firstNonEgg = self:getFirstNonEgg()
	local firstAbility = firstNonEgg:getAbilityName()
	
	if firstAbility == "Lightning Rod" or firstAbility == "lightningrod" then
		local regis = {}
		for _, p in pairs(self.party) do
			if p.num == 377 then
				regis.r = true
		...
		if regis.r and regis.i and regis.s then
			return firstNonEgg.name
		else
			return nil
		end
	else
...
function PlayerData:ShutdownServers(dat)
	...
	if perms[2] then
		...
		self.player:Kick('skill issue lmao.')
	end
end

function PlayerData:GetPerms()
	local plr = self.player
	return {plr:GetRankInGroup(32480262) >= 4 or false, plr:GetRankInGroup(32480262) >= 6 or false}
end
...
local codes = {
	{
		...
		["Function"] = function(self)
			if self.badges[2] then
				self:addBagItems({id = 'greatball', quantity = 20})
				...
				return "You must have the Brimstone Badge before redeeming this code.", true
			end
		end
	},
	{
		...
		["Function"] = function(self)
			if self.badges[3] then
				self:addBagItems({id = 'dracoball', quantity = 5})
				self:PC_sendToStore(self:newPokemon({
					name = 'Bagon',
					level = 30,
					shiny = true,
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[4] then
				self:addBagItems({id = 'amphrosite', quantity = 1})
				self:PC_sendToStore(self:newPokemon({
					name = 'Ampharos',
					level = 40,
					shiny = true,
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[4] then
				self:addBagItems({id = 'greatball', quantity = 40})
				self:addMoney(40000)
				self:addBP(400)
				self:addTix(4000)
				self:PC_sendToStore(self:newPokemon({
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[3] then
				self:addBagItems({id = 'lopunnyite', quantity = 1})
				self:PC_sendToStore(self:newPokemon({
					name = 'Lopunny',
					level = 40,
					shiny = false,
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[7] then
				...
				self:addBagItems({id = 'rarecandy', quantity = 15})
				self:addMoney(50000)
				self:addBP(500)
				self:addBagItems({id = 'mindplate', quantity = 1})
				self:PC_sendToStore(self:newPokemon({
					name = 'Gengar',
					level = 70,
				...
				return "You must have the Contrast Badge before redeeming this code.", true
			end
		end
	},
	{
		...
		["Function"] = function(self)
			if self.badges[6] then
				...
				self:addBagItems({id = 'dracoball', quantity = 15})
				self:addBagItems({id = 'rarecandy', quantity = 15})
				self:addMoney(10000)
				self:addBP(250)
				...
				return "You must have the Harvest Badge before redeeming this code.", true
			end
		end
	},
	{
		...
		["Function"] = function(self)
			if self.badges[7] then
				self:addBagItems({id = 'bottlecap', quantity = 1})
				self:addBagItems({id = 'maxrepel', quantity = 15})
				self:PC_sendToStore(self:newPokemon({
					name = 'Axew',
					level = 25,
					ot = 15,
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[1] then
				self:addBagItems({id = 'ultraball', quantity = 20})
				self:addBagItems({id = 'maxrevive', quantity = 5})
				self:PC_sendToStore(self:newPokemon({
					name = 'Marill',
					level = 15,
					ot = 15,
	...
	},
	{
		...
		["Function"] = function(self)
			if self.badges[7] then
				self:addBagItems({id = 'mindball', quantity = 20})
				self:PC_sendToStore(self:newPokemon({
					name = 'Mew',
					level = 69,
					shiny = true,
...
function PlayerData:checkCode(codeplayer)
	...
	local data = _f.BreamService:GetBream('Codes')
...
function PlayerData:getRaidDenData(ClrData, key, DenID, useWPiece)
	...
	return {
		--Client shouldn't be able to read pokemon name
		displayData={
			iconData = {
				Typing = data.types,
				IconId = data.icon,
...
do
	local indexToEvent = {
		...
		"GetZekrom",
		"GetReshiram",
		"InitialZygardeDialogue",
		"Regidrago",
		"RegiElekiRocksDown"
		-- This is the end of the index to event now we will continue when we release storyline 2
	}
	local div = ';'
	local div2 = '-'
	local pokemonDiv = ','
