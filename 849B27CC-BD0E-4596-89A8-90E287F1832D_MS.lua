...
local BreamService = _f.BreamService
local banlist = BreamService:GetBream('Banned')
...
local function playerNotificationListUpdated(player, list)
	stores:GetBream('Notification', tostring(player.UserId)):UpdateAsync('Inbox', function(list)
...
local function subscribePlayer(player)
	...
	playerSubscriptions[player.Name] = stores:GetBream('Notification', tostring(player.UserId)):OnUpdate('Inbox', function(list)
...
local function sendNotification(recipientId, n)
	...
	if recipientId then
		global = false
		recipientStore = stores:GetBream('Notification', tostring(recipientId))
	end
	n.ts = os.time()
	n.id = uid()
